...
function waitForWorkerEvent(worker: Worker, expectedType: WorkerEvent['type']): Promise<WorkerEvent> {
...
function spawnVirtualWorker(key: string, marker: string, delayMs = 0): Worker {
...
function spawnCrashedRegisterWorker(key: string): Worker {
...
async function concurrentStatus(port: number, path: string, count: number): Promise<number[]> {
...
async function constantConcurrencyStatus(
  port: number,
  path: string,
  concurrency: number,
  durationMs: number,
